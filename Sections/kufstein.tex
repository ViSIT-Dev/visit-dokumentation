...
Dieser Absatz gilt, wenn das Fernrohr bereits eingerichtet ist und nur neuer POI (Points of Interesst) hinzugefügt werden soll.\\
...
Als nächstes müssen in der dunkelblauen Leiste die Einstellungen ausgewählt und das Häckchen bei Kuratorenmodus gesetzt und gespeichert werden (siehe Abbildung \ref{img:fernrohr_einstellungen}). Danach kann bereits die Seite im Browser geöffnet werden. Dies geschieht über einen Rechtsklick auf Fernrohr im Seitenbaum und der Auswahl \glqq Show\grqq{} (siehe Abbildung \ref{img:fernrohr_browser_ansicht}). 
...
\subsection{Auslesen der x- und y-Koordinaten}
...
Sobald eines dieser POI anvisiert wird - dieses sich also im Mittelpunkt des Sichtfensters befindet - wird der Radium des POI's größer und der Text oder das Video (wird automatisch abgespielt, der Besucher muss nicht mehr auf Start drücken) werden angezeigt (siehe Abbildung .. SCREENSHOT!!!!). 
...
\subsection{Anlegen eines neuen Kartenelements}
...
Die beiden Überschriften werden nur benötigt, wenn es sich um einen Texttyp (also zum Beispiel ein Bild und dazu ein Text zum Lesen) handelt. Zum Texttyp gab es keine Designvorlage, dies wurde lediglich Vollständigkeit halber implementiert, grundsätzlich ist es dafür da, Fullscreen Videos abzuspielen. Sollte ein Texttyp verlangt werden, muss das entsprechende Design implementiert werden.\\
Bei Videos reicht nur eine Überschrift. Als nächstes muss die Position eingetragen werden. Hier müssen die zuvor notierten x- und y-Koordinaten aus dem Kuratorenkästchen eingetragen werden (siehe Abbildung \ref{img:fernrohr_x_u_y_koordinaten}). Handelt es sich um ein Fullscreen Video, dann muss das Häckchen daneben gesetzt werden. Als nächstes können Medien hinzugefügt werden.

\subsection{Größe (Radius) der POI's}
...
Dies ist wieder in den Einstellungen des Fernrohrs zu finden. Nach der Entfernung des Häckchens kann die Seite mittels Rechtsklick auf \glqq Fernrohr\grqq{} und der Auswahl von \glqq Show\grqq{} im Browser angezeigt werden. Jetzt ist das Kuratorenkästchen nicht mehr zu sehen, stattdessen befindet sich die Flagge der ausgewählten Sprache im oberen rechten Eck (siehe Abbildung SCREENSHOT!!!!!!!!!!!!!).
...
Das ViSIT-Projekt wurde am 1. Oktober 2016 gestartet. Dabei wurden auch einige Deadlines vereinbart, sodass mit der Entwicklung begonnen werden kann. Daraufhin wurde ab September 2018 bis Projektende ein Projektzeitplan erstellt, wo ersichtlich ist, war an was gerade arbeitet und wann was fertig sein wird (siehe Abbildung \ref{img:projektzeitplan}).


Eine solche Deadline - zum Beispiel das Feature Freeze Tablets -  war am 31.05.2018, damit ab Juni 2018 mit der Entwicklung der Applikationen begonnen werden konnte. Zu dem Zeitpunkt stand also das Design fest, welches die Applikationen am Ende des Projekts haben sollen. Damit konnte mit der Entwicklung des Front- und Backends begonnen werden.

Am 30.11.2018 wurden jedoch neue Layouts für die Tablets bekannt gegeben. Alles bis dahin entwickelte musste an das neue Layout angepasst werden, was viel Entwicklungszeit in Anspruch nahm. Als dies überarbeitet wurde, wurde am 21.05.2019 bekannt gegeben, dass ab sofort keine iPads verwendet werden sonder durch Lenovo Tablets ersetzt werden. Die überarbeiteten Layouts passten aber nicht für Lenovo Tablets und mussten neu gestaltet werden (29.05.2019). Auch jetzt musste der bereits zweimal überarbeitete Code wieder bearbeitet und auf Lenovo Tablets angepasst werden.\\



\begin{figure}[ht!]
\centering
\includegraphics[width=13cm]{Figures/paula/projektverlauf/projektzeitplan.png}
\caption{Projektzeitplan für das ViSIT-Projekt }
\label{img:projektzeitplan}
\end{figure}

Hier sind weitere Verzögerungen angeführt:



\begin{center}
\begin{tabular}{ c l }
 31.05.2018 & Feature Freeze Tablets \\ 
 31.07.2018 & Feature Freeze Fernrohr \\  
 30.11.2018 & Neue Layouts für Tablets in Zeplin \\ 
 21.05.2019 & iPads ersetzt durch Lenovo Tablets \\ 
 29.05.2019 & Nur noch neue Tablet Layouts für Tabltes (alte Obsolet) \\ 
 23.06.2019 & Fernrohr erhalten ohne Endstops und falschem Windows \\ 
 28.07.2019 & Server auf Festung eingerichtet (beschlossen im Meeting vom 09.04.2019) \\ 
 07.08.2019 & Fernrohr erhalten mit absoluten Drehgebern mit Overflow \\ 
 08.08.2019 & Netzwerk auf Festung noch nicht installiert (beschlossen im Meeting vom 9.4.2019) \\ 
 30.09.2019 & Projekt Ende 
\end{tabular}
\end{center}
